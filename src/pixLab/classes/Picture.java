...
//	public Picture(Picture Picture, String string) {
//		super (Picture, string);
//	}
	...
	public void mirrorVertical() {
		...
		for (int row = 0; row < pixels.length; row++) {
			for (int col = 0; col < width / 2 - 20; col++) {
		...
		for (int row = 0; row < pictureMatrix.length; row++) {
			...
			for (int col = 0; col < pictureMatrix[0].length; col++) {
				...
				if (row != col && row < pictureMatrix[0].length && col < pictureMatrix.length) {
					tr = pictureMatrix[row][col];
					bl = pictureMatrix[col][row];
		...
		for (int row = 0; row < shortest; row++) {
			for (int col = 0; col < shortest; col++) {
				if (row + col != shortest - 1) {
					tl = pictureMatrix[row][col];
					br = pictureMatrix[shortest - col - 1][shortest - row - 1];
					br.setColor(tl.getColor());
				}
			}
		}
	}
	...
	public void mirrorHorizantalBottomToTop() {
		...
		for (int col = 0; col < pixels[0].length; col--) {
			for (int row = 0; row < hight / 2 - 1; row++) {
	...
	public void mirrorGull() {
		...
		// loop through the rows
		for (int row = 230; row < 365; row++) {
			// loop from 13 to just before the mirror point
			for (int col = 215; col < 360; col++) {
	...
	public BufferedImage buffConverter() {
		Pixel pixel = null;
		Pixel[][] pixels = this.getPixels2D();
		Pixel[][] image = pixels;
	...
	public void edgeDetectionV2(int edgeDist) {
		Pixel leftPixel = null;
		Pixel rightPixel = null;
		...
		for (int row = 0; row < pixels.length; row++) {
			for (int col = 0; col < pixels[0].length - 2; col++) {
				...
				if (leftPixel.colorDistance(rightColor) > edgeDist
						&& leftPixel.colorDistance(secondRightColor) > edgeDist)
					leftPixel.setColor(Color.BLACK);
				else
					leftPixel.setColor(Color.WHITE);
			}
